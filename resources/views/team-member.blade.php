...
<!-- OTHER TEAM MEMBERS -->
<div class="section-divider" style="padding:0;margin:2rem auto 0;"></div>
...
function renderProfile(member) {
  ...
  if (member.bio) {
    leftHtml += `
      <div class="reveal">
        <p style="font-size:1.1rem;line-height:1.8;color:var(--w80);margin-bottom:2.5rem;white-space:pre-wrap;">${member.bio}</p>
      </div>
    `;
  }

  if (member.specialistFocus && member.specialistFocus.length > 0) {
    leftHtml += `
      <div class="reveal" style="margin-bottom:2.5rem;">
        <h3 style="font-size:1.4rem;font-weight:600;margin-bottom:1.25rem;color:var(--white);">Specialist Focus</h3>
        <div style="display:flex;gap:0.75rem;flex-wrap:wrap;">
          ${member.specialistFocus.map(tag => `<span style="background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.08);padding:0.4rem 1.1rem;border-radius:2rem;font-size:0.9rem;color:var(--w90);letter-spacing:0.02em;">${tag}</span>`).join('')}
        </div>
      </div>
    `;
  }

  if (member.personalStatement) {
    leftHtml += `
      <div class="reveal" style="margin-bottom:2.5rem;">
        <h3 style="font-size:1.4rem;font-weight:600;margin-bottom:1.25rem;color:var(--white);">Personal Statement</h3>
        <blockquote style="border-left:4px solid var(--accent);padding:1.25rem 1.5rem;background:rgba(255,255,255,0.02);margin:0;border-radius:0 8px 8px 0;">
          <p style="font-size:1.15rem;font-style:italic;line-height:1.7;color:var(--w80);margin:0;">
