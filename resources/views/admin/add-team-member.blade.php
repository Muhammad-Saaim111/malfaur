    ...
    <div class="form-left">
      ...
      <div class="form-section">
        ...
        <div class="section-body">
          ...
          <div class="form-row single">
            <div class="form-group">
              <label>Specialist Focus</label>
              <div class="tag-input-wrap" style="display:flex;gap:0.5rem;align-items:center;">
                <input type="text" id="specialistInput" placeholder="e.g. User Experience, Agile..." onkeypress="if(event.key==='Enter'){event.preventDefault(); addTag('specialistInput', 'specialistTags', 'specialist_focus');}" style="flex:1;" />
                <button type="button" class="btn" style="padding:0.75rem 1.25rem;background:var(--accent);color:var(--white);border:none;border-radius:var(--radius-sm);cursor:pointer;" onclick="addTag('specialistInput', 'specialistTags', 'specialist_focus')">+</button>
              </div>
              <div class="tags-container" id="specialistTags" style="margin-top:0.75rem;display:flex;gap:0.5rem;flex-wrap:wrap;"></div>
            </div>
          </div>
          ...
          <div class="form-row single">
            <div class="form-group">
              ...
              <textarea id="personalStatement" name="personal_statement" rows="2" placeholder="Short quote text..."></textarea>
            </div>
          </div>
...
function addTag(inputId, displayId, storeKey) {
  ...
  const hiddenEl = hiddenMap[storeKey] ? document.getElementById(hiddenMap[storeKey]) : null;
  if (hiddenEl) hiddenEl.value = JSON.stringify(tagStores[storeKey]);
}
...
async function publishMember() {
  ...
  try {
    ...
    formData.append('specialist_focus', JSON.stringify(tagStores['specialist_focus'] || []));
    
    // Personal Statement
    formData.append('personal_statement', document.getElementById('personalStatement').value);
...
if (isEditMode && editingMember) {
  const fieldMap = {
    ...
    personalStatement: editingMember.personalStatement,
  };
  ...
  tagStores.specialist_focus = Array.isArray(editingMember.specialistFocus) ? editingMember.specialistFocus : [];
  renderTags('skillTags', 'skills');
  renderTags('expertiseTags', 'expertise');
  renderTags('specialistTags', 'specialist_focus');
